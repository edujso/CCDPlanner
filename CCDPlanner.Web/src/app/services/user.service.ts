...
export class UserService {
  ...
  getAll() {
    ...
    addClaim(userId: string, claim: Claim) {
      this._spinnerService.show();
      return this.http.post('/api/users/addClaim/' + userId, claim, this.jwt())
        .map((response: Response) => response.json())
        .catch(this._exceptionService.catchBadResponse)
        .finally(() => this._spinnerService.hide());
    }

    removeClaim(userId: string, claim: Claim) {
      this._spinnerService.show();
      return this.http.post('/api/users/removeClaim/' + userId, claim, this.jwt())
        .map((response: Response) => response.json())
        .catch(this._exceptionService.catchBadResponse)
        .finally(() => this._spinnerService.hide());
    }
